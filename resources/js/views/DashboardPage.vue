  ...
  <div :class="['flex h-screen', isDarkMode ? 'dark' : '']">
    ...
    <template v-else>
      ...
      <main :class="['flex-1 overflow-y-auto custom-scrollbar', isDarkMode ? 'bg-gray-900' : 'bg-slate-50']">
        ...
        <!-- ORIENTATION -->
        <div v-if="activeModule === 'orientation'" class="p-3 md:p-8"><p class="text-2xl font-black text-slate-950 mb-6"><i class="fas fa-chart-line text-primary mr-2"></i>Bilan d'orientation</p><div class="grid md:grid-cols-3 gap-6 mb-8"><router-link v-for="test in orientationTests" :key="test.id" :to="test.link" class="bg-white rounded-2xl border border-slate-100 p-6 hover:shadow-xl hover:border-primary/30 transition-all no-underline text-slate-950"><div class="w-12 h-12 rounded-full flex items-center justify-center mb-3" :class="test.bgColor"><i :class="[test.icon, test.iconColor, 'text-xl']"></i></div><h3 class="font-bold text-lg">{{ test.name }}</h3><p class="text-sm text-gray-500 mt-1">{{ test.description }}</p><div class="mt-3 flex justify-between items-center"><span class="text-xs text-gray-400"><i class="far fa-clock mr-1"></i>{{ test.duration }}</span><span class="text-primary text-sm font-medium">Commencer <i class="fas fa-arrow-right ml-1"></i></span></div></router-link></div><div class="bg-white rounded-2xl border border-slate-100 p-6"><h3 class="font-bold text-lg mb-4"><i class="fas fa-chart-simple text-primary mr-2"></i>Résultats des tests</h3><div class="space-y-4"><div v-for="result in orientationResults" :key="result.id" class="flex justify-between items-center p-3 bg-slate-50 rounded-xl"><div><p class="font-medium">{{ result.testName }}</p><p class="text-xs text-gray-500">Complété le {{ result.date }}</p></div><span class="px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700"><i class="fas fa-check-circle mr-1"></i>Complété</span></div></div></div></div>
...
const startExam = (exam) => { router.push('/examens'); };
const startQuiz = (quiz) => { router.push('/ressources'); };
const applyForScholarship = (scholarship) => { window.open('/mobilite/dossier', '_blank'); };
const goToStudyGroups = () => { router.push('/study-groups'); };
